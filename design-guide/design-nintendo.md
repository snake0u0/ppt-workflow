...
- **Pale Sky** (`{colors.sky}` — #9fbee7): The secondary-nav strip fill.
- **Canvas Soft** (`{colors.canvas-soft}` — #9fbee7): Light inset panel fills (same pale-sky value, kept as its own token for template mapping).
...
- **Hairline** (`{colors.hairline}` — #9aa7c9): The 1px cool border on form fields and thumbnail frames where chrome-indigo would be too heavy.

### Text
- **Carbon Navy** (`{colors.ink}` — #21242e): Primary text on light chrome.
- **Carbon** (`{colors.carbon}` — #21242e): The same carbon navy as a *fill* — the dark command layer (nav bar, rail buttons, footer).
- **Chrome Indigo Soft** (`{colors.ink-soft}` — #3d4f97): Secondary text and small-caps chrome labels.
...
Arial is a system font and needs no bundling. Korean text must not fall to a random system default: always chain a Korean-capable face behind it — `Arial, 'Helvetica Neue', 'Pretendard', 'Apple SD Gothic Neo', sans-serif`. Pretendard is auto-injected by the viewer (`/shared/fonts.css`); the repo's bundled fonts are Pretendard, Inter, and Paperlogy — no pixel font is bundled, so treat Silkscreen/VT323 pixelation as optional flavor, not a requirement. For the display wordmarks use **Arial Black** (system) or Paperlogy at weight 900, with a CSS `-webkit-text-stroke` outline and a hard offset `text-shadow` — the outline + shadow matters more than the exact face.
...
- Pressed state — fill deepens to `{colors.nav-gold}`.
...
- A white `{colors.surface}` field with `{colors.ink}` text in `{typography.body}`, `{rounded.xs}` corners, and a hard 1px `{colors.carbon}` border (sharper than the inputs) closing on a beveled native dropdown-chevron button at the right edge. ~24px tall, `{spacing.xs}` padding.

**`field-label`** — Inline form label
- A bold `{colors.ink}` label in `{typography.link}` (Arial Bold 12px) sitting left of its input. `{rounded.none}`, `{spacing.xxs}` padding.
...
- A `{colors.platinum}` light-gray `{rounded.md}` panel holding a form, capped by a `{components.section-label-bar}` header (≡ LOG IN / ≡ REGISTER). `{spacing.lg}` interior.

**`dotted-divider`** — Dotted hairline rule
- A 1px dotted `{colors.muted-indigo}` separator rule used between form sub-sections. A recurring Y2K-chrome detail — dotted rather than solid.
...
- A `{colors.carbon}`-framed `{rounded.sm}` tile (~95×60px) holding a screenshot thumbnail with a `{typography.micro}` URL caption. Tight `{spacing.xxs}` padding, arranged in a 2×2 grid.
...
**`system-tile`** — Hardware grid tile
...
**`link-row-card`** — Utility link card
...
- A white `{rounded.sm}` grid calendar in `{typography.micro}`, with a `{colors.carbon}` caption header, highlighted event dates, and a month-select dropdown. `{spacing.sm}` padding.
...
This guide describes a fixed-canvas *website*; our decks are fixed-canvas **1280×720 slides**, which suits the metaphor perfectly — treat every slide as one machine faceplate. Deck-wide rules (draft fidelity, `---` slide boundaries, `@claude` directives, `※` footnotes) come from `.claude/deck-build-core.md`; this section only maps the web chrome onto slide chrome.

- **Slide body** = the `{colors.canvas}` periwinkle faceplate. Chamfer the outer corners of the largest plate on cover-class slides; content slides may keep a plain full-bleed periwinkle body carrying inset plates.
- **Cover slide** = the `hero-panel`: a full-bleed tinted field (`{colors.lavender}` default) under an outlined, hard-shadowed `{typography.display}` deck title, with the presenter/affiliation line set as a `mascot-bubble`-style white plate or a carbon `footer-bar` strip. If the draft supplies a cover image via `![]()`, it becomes the hero photography; otherwise use the flat tinted field with the halftone/circuit texture drawn in CSS — never invent photography.
- **Section header on content slides** = the `nav-bar` voice: a `{colors.carbon}` slab strip carrying the slide title in `{colors.nav-gold}` uppercase `{typography.nav-link}` treatment (scaled up for 1280×720 legibility — roughly 22–26px), optionally with a small `logo-pill`-style page badge.
- **Content panels** = beveled plates: `section-label-bar` caps over `{colors.platinum}` or `{colors.surface}` bodies. Bullets can sit as `news-row` list rows with `{colors.signal}` chevron chips as the row markers.
- **Figures** sit inside `featured-tile` frames: 1–2px `{colors.chrome-indigo}` edges, `{rounded.sm}`, on platinum or white insets. Multiple figures share a tight tiled grid, not an airy row.
- **Footnotes (`※`)** live in a thin `footer-bar`-toned strip or as `{typography.micro}` platinum inset text — still small and quiet, per the core rules.
- **Scale up type for projection.** The site's 10–13px web sizes are too small at 1280×720 presentation distance; keep the *ratios* and treatments (uppercase tracking, outlined display) but let body run ~15–17px, `ui-label` ~12–14px, display 44–64px.
- Warm colors stay rationed: `{colors.signal}` only on forward cues (arrow chips, emphasis markers), `{colors.amber}` on badges/labels, `{colors.nav-gold}` on header words. Never tint body copy warm.
- **Interactive showcase slides** (a slide whose content *is* an HTML effect demo): the house effect is a **typed box-art wordmark + reactive LED equalizer** — the sentence types itself in the outlined `{typography.display}` treatment with a `{colors.signal}` caret, while a bottom row of segmented meter bars (rAF-driven sine synthesis + typing energy + mouse response) rises through the console-audio-meter ramp `{colors.chrome-indigo}` → `{colors.periwinkle}` → `{colors.sky}` → `{colors.amber}` → `{colors.signal}`. This is the one sanctioned decorative use of the warm colors, because the effect itself is the slide's content; respect `prefers-reduced-motion` (show the full sentence, freeze the bars).
...
- Render hero wordmarks as **outlined + drop-shadowed `{typography.display}`** over full-bleed fields tinted per page.
